...
DistLibraryFile := $(if $(strip $(CppFile)),$(DistDir)/$(Target).a)
InstallLibDir := $(if $(strip $(CppFile)),$(User)/Lib)
...
install: $(DistLibraryFile)
	...
	@$(if $(strip $(DistLibraryFile)),mkdir -p $(InstallLibDir))
	@mkdir -p $(InstallIncDir)
	@$(if $(strip $(DistLibraryFile)),cp $< $(InstallLibDir)/)
